...
from coco_loder_online_enc_no_attr import CocoDataset, collate_fn
import json
import os
import opts
...
res = []
with torch.no_grad():
    for idx, batch in tqdm(enumerate(data), mininterval=2,
                           desc='  -(evalution test)  ', leave=False, total=len(data)):

        att_feat, fc_feat, img_ids = batch
        ...
        pred_cap = trans2sentence(preds, dataset.ix_to_word)

        for i, img_id in enumerate(img_ids):
            cap_dict = {}
            cap_dict['image_id'] = int(img_id)
            cap_dict['caption'] = pred_cap[i]
            res.append(cap_dict)


path = 'gen/online_val_1217.json'
