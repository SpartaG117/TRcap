...
class CocoDataset(Dataset):
    ...
    def __init__(self, opt, mode='train'):
        ...
        self.aug_seq_per_img = opt.aug_seq_per_img
        ...
        with open(opt.aug_json, 'r') as f:
            self.data_aug = json.load(f)
    ...
    def get_word_to_ix(self):
        word_to_ix = {}
        for key in self.ix_to_word.keys():
            word_to_ix[self.ix_to_word[key]] = int(key)
        return word_to_ix

    def process_aug_data(self, ix):

        aug_cap = self.data_aug[str(self.info['images'][ix]['id'])]
        num_aug = len(aug_cap)
        if num_aug == 0:
            return []
        seqs = []
        for i, cap in enumerate(aug_cap):
            if cap == '':
                continue
            tokens = cap.strip().split(' ')[:self.opt.max_length]
            seq = []
            for j, w in enumerate(tokens):
                seq.append(self.word_to_ix[w])
            seqs.append(seq)
        return seqs


    def get_captions(self, ix, seq_per_img, aug_seq_per_img=5):
        ...
        # augment data
        if self.mode == 'train':
            aug_cap = self.process_aug_data(ix)

            if len(aug_cap) < aug_seq_per_img:
                if len(aug_cap) != 0:
                    aug_src_seq = []
                    aug_tgt_seq = []
                    for cap in aug_cap:
                        aug_src_seq.append([9488] + cap + [0] * (self.seq_length - len(cap)))
                        aug_tgt_seq.append(cap + [9489] + [0] * (self.seq_length - len(cap)))

                    aug_src_seq = torch.Tensor(aug_src_seq).long()
                    aug_tgt_seq = torch.Tensor(aug_tgt_seq).long()
                    extra_src_seq = []
                    extra_tgt_seq = []
                    for _ in range(aug_seq_per_img - len(aug_cap)):
                        id = random.randint(0, seq_per_img-1)
                        extra_src_seq.append(src_seq[id])
                        extra_tgt_seq.append(tgt_seq[id])

                    extra_src_seq = torch.stack(extra_src_seq, dim=0)
                    extra_tgt_seq = torch.stack(extra_tgt_seq, dim=0)

                    src_seq = torch.cat([src_seq, aug_src_seq, extra_src_seq], dim=0)
                    tgt_seq = torch.cat([tgt_seq, aug_tgt_seq, extra_tgt_seq], dim=0)

                else:
                    extra_src_seq = []
                    extra_tgt_seq = []
                    for _ in range(aug_seq_per_img):
                        id = random.randint(0, seq_per_img - 1)
                        extra_src_seq.append(src_seq[id])
                        extra_tgt_seq.append(tgt_seq[id])

                    extra_src_seq = torch.stack(extra_src_seq, dim=0).long()
                    extra_tgt_seq = torch.stack(extra_tgt_seq, dim=0).long()
                    src_seq = torch.cat([src_seq, extra_src_seq], dim=0)
                    tgt_seq = torch.cat([tgt_seq, extra_tgt_seq], dim=0)

            else:
                ixl = random.randint(0, len(aug_cap) - aug_seq_per_img)
                aug_cap_s = aug_cap[ixl:ixl + aug_seq_per_img]
                aug_src_seq = []
                aug_tgt_seq = []
                for cap in aug_cap_s:
                    aug_src_seq.append([9488] + cap + [0] * (self.seq_length - len(cap)))
                    aug_tgt_seq.append(cap + [9489] + [0] * (self.seq_length - len(cap)))
                ...
                src_seq = torch.cat([src_seq, aug_src_seq], dim=0)
                tgt_seq = torch.cat([tgt_seq, aug_tgt_seq], dim=0)
    ...
    def __getitem__(self, ix):
        ...
        src_seq, tgt_seq, gts, raw_gts = self.get_captions(index, self.seq_per_img, self.aug_seq_per_img)
...
if __name__ == '__main__':
    opt = opts.parse_opt()
    train_dataset = CocoDataset(opt, 'train')
